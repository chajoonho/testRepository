...
function FileInput({ inputName, setFile, value }) {
  ...
  useEffect(() => {
    ...
    // ObjectURL 객체를 사용하여 미리보기 기능을 구현할 수 잇다.
    // OnjectURL을 만들면 웹 브라우저에 메모리를 할당한다.
    ...
    // 디펜던시 리스트에 있는 값이 바뀌면 다시 함수를 실행하는데 이 전에 이랙트는
  ...
  // ReviewForm 렌더링 => FileInput 렌더링되면서 useEffect실행
  // useEffect 내부 코드가 실행되고 사진 변경
  // =>ReviewForm 재 렌더링 => FileInput 도 재 렌더링 => 이때에는 useEffect내부 코드가 실행되는게 아니다.
